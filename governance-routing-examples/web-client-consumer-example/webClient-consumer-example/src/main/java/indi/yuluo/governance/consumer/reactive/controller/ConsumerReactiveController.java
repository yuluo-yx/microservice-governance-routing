...
package indi.yuluo.governance.consumer.reactive.controller;
...
import indi.yuluo.governance.consumer.reactive.configuration.ConsumerWebClientConfiguration;
...
public class ConsumerReactiveController implements ApplicationContextAware {
		...
		for (String service : services) {
			...
			for (ServiceInstance instance : instances) {
				if ((instance.getPort() + "").equals(serverPort)) {
					...
					nmap.put(GovernanceRoutingConstants.PORT,
							Collections.singletonList(instance.getPort() + ""));
					nmap.put(GovernanceRoutingConstants.HOST,
							Collections.singletonList(instance.getHost()));
					nmap.put(GovernanceRoutingConstants.INSTANCE_ID,
	...
	public Mono<String> routerTest() {
		...
		return webClientBuilder.build().get()
				.uri(GovernanceRoutingConstants.SERVICE_PROVIDER_ADDRESS + "/test-a1")
